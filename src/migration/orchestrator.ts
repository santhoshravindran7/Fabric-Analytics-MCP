...
export class MigrationOrchestrator {
   ...
   */
  private async getFabricPricing(region: string = 'eastus'): Promise<Record<string, number>> {
    ...
    try {
      ...
      const cuPriceItem = items.find((item: any) => 
        item.meterName && item.meterName.includes('Capacity Usage CU')
      );
   ...
   */
  private getFallbackFabricPricing(): Record<string, number> {
   ...
   */
  async getSynapseWorkspaceDetails(subscriptionId: string, resourceGroup: string, workspaceName: string): Promise<any> {
   ...
   */
  async getSynapseComputeSpend(subscriptionId: string, resourceGroup: string, workspaceName: string, startDate?: string, endDate?: string): Promise<any> {
   ...
   */
  async migrateSparkPoolsToFabric(sparkPools: any[], targetCapacitySku: string, workspaceId: string): Promise<any> {
    ...
    const validation = sparkPoolManager.validateCapacityForPools(
      recommendations,
      capacitySpecs.sparkVCores,
      capacitySpecs.maxSparkVCoresWithBurst
    );
   ...
   */
  async recommendFabricCapacity(synapseComputeSummary: any, minSku?: string, maxSku?: string): Promise<any> {
    ...
    const availableSkus = allSkus.slice(
      Math.max(0, minIndex),
      Math.min(allSkus.length, maxIndex + 1)
    );
    ...
    const nodeVCoreMap: Record<string, number> = {
      'Small': 4,
      'Medium': 8,
      'Large': 16,
      'XLarge': 32,
  ...
  private discovery: SynapseDiscovery;
  private transformer: NotebookTransformer;
  private provisioner?: FabricProvisioner;
  ...
  private generateMigrationReport(
    ...
    transformResults: TransformationResult[],
    provisionedIds: Map<string, string>,
    validation: { valid: boolean; issues: string[] },
    duration: number
  ): MigrationReport {
    const details: MigrationItemResult[] = [];

    // Create result for each notebook
    for (const result of transformResults) {
      const fabricItemId = provisionedIds.get(result.notebookName);
      
      let status: 'success' | 'failed' | 'manual_review' = 'failed';
   ...
   */
  async discoverOnly(source: SynapseSource): Promise<MigrationInventory> {
   ...
   */
  async transformOnly(inventory: MigrationInventory): Promise<TransformationResult[]> {
   ...
   */
  async listSynapseWorkspaces(subscriptionId?: string): Promise<any[]> {
