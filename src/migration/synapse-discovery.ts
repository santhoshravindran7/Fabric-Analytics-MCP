...
export class SynapseDiscovery {
   ...
   */
  async listWorkspaces(subscriptionId?: string): Promise<any[]> {
    try {
      const subId = subscriptionId || await this.getCurrentSubscription();
      const { stdout } = await execAsync(
        `az synapse workspace list --subscription ${subId} --output json`
      );
   ...
   */
  private async getCurrentSubscription(): Promise<string> {
   ...
   */
  async discoverWorkspace(source: SynapseSource): Promise<MigrationInventory> {
   ...
   */
  private async discoverNotebooks(
   ...
   */
  private async discoverLinkedServices(
    source: SynapseSource,
    token: string
  ): Promise<LinkedServiceAsset[]> {
    try {
      const apiVersion = '2021-06-01-preview';
      ...
      const data = await response.json();
      const linkedServices: LinkedServiceAsset[] = [];
   ...
   */
  private async discoverSparkJobs(
    source: SynapseSource,
    token: string
  ): Promise<SparkJobAsset[]> {
    try {
      const apiVersion = '2021-06-01-preview';
   ...
   */
  async exportInventory(inventory: MigrationInventory, outputPath: string): Promise<void> {
