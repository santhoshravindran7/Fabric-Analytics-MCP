...
export function registerCapacityTools(
  server: McpServer,
  authConfig: { defaultWorkspaceId?: string; method: AuthMethod },
  executeApiCall: <T>(
    bearerToken: string | undefined,
    workspaceId: string,
    operation: string,
    apiCall: (_client: any) => Promise<{ status: 'success' | 'error'; data?: T; error?: string }>,
  ...
  const existingTools: any[] = (server as any).listTools ? (server as any).listTools() : [];
  const existing = new Set(existingTools.map((t: any) => t.name));
  ...
  server.tool(
    ...
    async ({ bearerToken }) => {
      ...
      const capacities: any[] = Array.isArray(result.data) ? (result.data as any[]) : [];
