...
class ToolValidator {
  ...
  async validateToolRegistration() {
    ...
    // Look for MCP SDK tool registrations with server.tool()
    ...
    // Also check for specific tool names to verify they're present
    const toolNames = Object.values(TOOL_CATEGORIES).flat();
    const foundTools = toolNames.filter(toolName => 
      indexContent.includes(`"${toolName}"`) || indexContent.includes(`'${toolName}'`)
    );
    
    console.log(`Found ${foundTools.length} specific tool names in code`);
    
    if (toolRegistrations >= totalExpected - 6) { // Allow for reasonable variance due to duplicates or missing tools
      console.log('✅ Tool registration count looks good');
      return true;
    } else {
      console.log('❌ Insufficient tool registrations found');
      console.log(`   Expected: ${totalExpected}, Found: ${toolRegistrations}`);
      
      // Show more details for debugging
      console.log('\n   Debug info:');
      console.log(`   - server.tool() calls: ${toolRegistrations}`);
      console.log(`   - Specific tool names found: ${foundTools.length}`);
