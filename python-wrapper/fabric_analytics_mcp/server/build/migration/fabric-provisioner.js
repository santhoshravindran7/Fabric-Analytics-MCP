...
export class FabricProvisioner {
    ...
    async getAccessToken() {
        ...
        try {
            ...
            const { stdout } = await execAsync('az account get-access-token --resource https://api.fabric.microsoft.com --query accessToken --output tsv');
     ...
     */
    createProvisioningPlan(results) {
        const items = [];
     ...
     */
    async provisionNotebooks(results) {
        ...
        for (const result of successfulResults) {
            try {
                const notebookId = await this.createNotebook(result.notebookName, result.transformedContent, token);
     ...
     */
    async createNotebook(name, content, token) {
     ...
     */
    async createShortcut(sourcePath, targetPath, shortcutName) {
        if (!this.lakehouseId) {
    ...
    async executeProvisioningPlan(plan) {
        ...
        for (const item of notebookItems) {
            try {
                const notebookId = await this.createNotebook(item.name, item.transformedContent, await this.getAccessToken());
     ...
     */
    async validateProvisionedItems(itemIds) {
        const issues = [];
        if (this.config.dryRun) {
...
//# sourceMappingURL=fabric-provisioner.js.map