...
export class SparkPoolManager {
    async getAzureAccessToken() {
        const tokenOutput = execSync('az account get-access-token --resource https://management.azure.com --query accessToken -o tsv', { encoding: 'utf-8' });
        return tokenOutput.trim();
    }
    /**
     * List all Spark pools in a Synapse workspace
     */
    async listSynapseSparkPools(subscriptionId, resourceGroup, workspaceName) {
     ...
     */
    convertSynapseToFabricPools(synapsePools, capacitySku) {
     ...
     */
    validateCapacityForPools(recommendations, capacitySparkVCores, capacityMaxBurst) {
        const warnings = [];
        let canSupport = true;
        for (const rec of recommendations) {
            ...
            if (requiredVCores > capacityMaxBurst) {
                canSupport = false;
                warnings.push(`❌ Pool '${rec.synapsePoolName}' requires ${requiredVCores} vCores but capacity only provides ${capacityMaxBurst} (with burst)`);
            }
            else if (requiredVCores > capacitySparkVCores) {
                warnings.push(`⚠️ Pool '${rec.synapsePoolName}' requires ${requiredVCores} vCores, using burst capacity (base: ${capacitySparkVCores}, burst: ${capacityMaxBurst})`);
            }
            else {
                warnings.push(`✅ Pool '${rec.synapsePoolName}' can run within base capacity (${requiredVCores}/${capacitySparkVCores} vCores)`);
            }
        }
        return { canSupport, warnings };
    }
}
//# sourceMappingURL=spark-pool-tools.js.map