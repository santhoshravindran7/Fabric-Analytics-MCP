...
import { MicrosoftAuthClient, AuthMethod } from './auth-client.js';
import http from 'http';
import url from 'url';
// Migration tools
import { migrationTools, ListSynapseWorkspacesSchema, DiscoverSynapseWorkspaceSchema, TransformNotebooksSchema, MigrateSynapseToFabricSchema, CreateFabricLakehouseSchema, ProvisionNotebooksSchema, ListFabricCapacitiesSchema, AssignCapacityToWorkspaceSchema, GetSynapseSparkPoolsSchema, CreateFabricSparkPoolsSchema, SynapseWorkspaceDetailsSchema, SynapseComputeSpendSchema, MigrateSparkPoolsToFabricSchema, RecommendFabricCapacitySchema } from './migration/mcp-tools.js';
/**
 * STDOUT SAFETY GUARD
 * -------------------------------------------------------------
...
function loadAuthConfig() {
    const method = process.env.FABRIC_AUTH_METHOD || AuthMethod.BEARER_TOKEN;
    return {
        method,
...
async function performAuthentication() {
    ...
    switch (authConfig.method) {
        case AuthMethod.SERVICE_PRINCIPAL:
            ...
            return await authClient.authenticateWithServicePrincipal(authConfig.clientId, authConfig.clientSecret, authConfig.tenantId);
        case AuthMethod.DEVICE_CODE:
            return await authClient.authenticateWithDeviceCode(authConfig.clientId, authConfig.tenantId);
        case AuthMethod.INTERACTIVE:
            return await authClient.authenticateInteractively(authConfig.clientId, authConfig.tenantId);
 ...
 */
function analyzeRunsSummary(runs) {
 ...
 */
function analyzeSubactivities(subactivities) {
 ...
 */
function filterRecentRuns(runs, hoursBack) {
 ...
 */
function analyzeSingleRun(run, subactivities) {
 ...
 */
function generatePerformanceRecommendations(run, subactivities, analysisType) {
    const recommendations = [];
    if (run.status === "Failed") {
        recommendations.push("🔍 Review error logs and consider retry mechanisms");
    }
    const longRunning = subactivities.filter(a => a.endTime && calculateDuration(a.startTime, a.endTime) > 300000);
 ...
 */
function _getNotebookTemplate(template) {
    const templates = {
        blank: {
            nbformat: 4,
            nbformat_minor: 5,
...
async function executeApiCall(bearerToken, workspaceId, operation, apiCall, simulationParams) {
...
server.tool("fabric_list_capacities", "List all available Fabric capacities", ListCapacitiesSchema.shape, async ({ bearerToken }) => {
    const result = await executeApiCall(bearerToken, authConfig.defaultWorkspaceId || "global", "list-capacities", async (client) => await client.listCapacities(), {});
...
server.tool("fabric_assign_workspace_to_capacity", "Assign a workspace to a dedicated Fabric capacity", AssignWorkspaceToCapacitySchema.shape, async ({ bearerToken, capacityId, workspaceId }) => {
    const result = await executeApiCall(bearerToken, authConfig.defaultWorkspaceId || "global", "assign-workspace-to-capacity", async (client) => await client.assignWorkspaceToCapacity(capacityId, workspaceId), { capacityId, workspaceId });
    if (result.status === 'error') {
...
server.tool("fabric_unassign_workspace_from_capacity", "Unassign a workspace from its capacity (move to shared capacity)", UnassignWorkspaceFromCapacitySchema.shape, async ({ bearerToken, workspaceId }) => {
    const result = await executeApiCall(bearerToken, authConfig.defaultWorkspaceId || "global", "unassign-workspace-from-capacity", async (client) => await client.unassignWorkspaceFromCapacity(workspaceId), { workspaceId });
    if (result.status === 'error') {
...
server.tool("fabric_list_capacity_workspaces", "List all workspaces assigned to a specific capacity", ListCapacityWorkspacesSchema.shape, async ({ bearerToken, capacityId }) => {
    const result = await executeApiCall(bearerToken, authConfig.defaultWorkspaceId || "global", "list-capacity-workspaces", async (client) => await client.listCapacityWorkspaces(capacityId), { capacityId });
    if (result.status === 'error') {
...
// ====================================
server.tool("fabric_list_synapse_workspaces", ListSynapseWorkspacesSchema.shape, {
    ...
    try {
        ...
        result.workspaces.forEach((ws, idx) => {
...
server.tool("fabric_discover_synapse_workspace", DiscoverSynapseWorkspaceSchema.shape, {
    ...
    try {
        ...
        if (result.notebooks.length > 0) {
            ...
            result.notebooks.forEach((nb, idx) => {
        ...
        if (result.pipelines.length > 0) {
            ...
            result.pipelines.forEach((p, idx) => {
                output += `${idx + 1}. **${p.name}**\n`;
                output += `   - ID: \`${p.id}\`\n`;
            });
            output += `\n`;
        ...
        if (result.linkedServices.length > 0) {
            ...
            result.linkedServices.forEach((ls, idx) => {
        ...
        if (result.sparkJobs.length > 0) {
            ...
            result.sparkJobs.forEach((sj, idx) => {
...
server.tool("fabric_transform_notebooks", TransformNotebooksSchema.shape, {
...
server.tool("fabric_migrate_synapse_to_fabric", MigrateSynapseToFabricSchema.shape, {
...
server.tool("fabric_create_lakehouse", CreateFabricLakehouseSchema.shape, {
...
server.tool("fabric_provision_notebooks", ProvisionNotebooksSchema.shape, {
...
server.tool("fabric_list_capacities", ListFabricCapacitiesSchema.shape, {
...
server.tool("fabric_assign_capacity", AssignCapacityToWorkspaceSchema.shape, {
...
server.tool("fabric_get_synapse_spark_pools", GetSynapseSparkPoolsSchema.shape, {
...
server.tool("fabric_create_fabric_spark_pools", CreateFabricSparkPoolsSchema.shape, {
...
server.tool("fabric_synapse_workspace_details", SynapseWorkspaceDetailsSchema.shape, {
...
server.tool("fabric_synapse_compute_spend", SynapseComputeSpendSchema.shape, {
...
server.tool("fabric_migrate_spark_pools_to_fabric", MigrateSparkPoolsToFabricSchema.shape, {
...
server.tool("fabric_recommend_fabric_capacity", RecommendFabricCapacitySchema.shape, {
 ...
 */
async function calculateHealthMetrics(bearerToken, workspaceId, dataflowId) {
 ...
 */
async function checkConnectivity(bearerToken, workspaceId, dataflowId) {
    try {
        const url = `https://api.fabric.microsoft.com/v1/workspaces/${workspaceId}/dataflows/${dataflowId}`;
        const response = await fetch(url, {
            headers: { 'Authorization': `Bearer ${bearerToken}` }
 ...
 */
async function checkConfiguration(bearerToken, workspaceId, dataflowId) {
    try {
        const url = `https://api.fabric.microsoft.com/v1/workspaces/${workspaceId}/dataflows/${dataflowId}`;
        const response = await fetch(url, {
            headers: { 'Authorization': `Bearer ${bearerToken}` }
 ...
 */
async function checkPerformance(bearerToken, workspaceId, dataflowId) {
 ...
 */
async function checkReliability(bearerToken, workspaceId, dataflowId) {
 ...
 */
async function checkResourceUsage(bearerToken, workspaceId, dataflowId) {
    return { status: 'healthy', score: 75, message: 'Resource usage appears normal' };
}
/**
 * Analyze trends (placeholder)
 */
async function analyzeTrends(bearerToken, workspaceId, dataflowId) {
...
function getHealthMessage(score) {
    ...
    if (score >= 80)
        return 'Good health';
    if (score >= 70)
        return 'Fair health, minor issues';
    if (score >= 60)
 ...
 */
function checkForAlerts(healthCheck) {
    const alerts = [];
    const score = healthCheck.overallHealth.score;
    if (score < 60) {
 ...
 */
function sortDataflows(dataflows, sortBy) {
    dataflows.sort((a, b) => {
        switch (sortBy) {
            case 'name':
            ...
            case 'status':
                ...
                return (statusOrder[b.health?.status] || 0) - (statusOrder[a.health?.status] || 0);
            default:
                return 0;
        }
    });
 ...
 */
function generateExecutiveSummary(statusData, healthCheck) {
...
server.tool("list-fabric-items", "List items in a Microsoft Fabric workspace", ListItemsSchema.shape, async ({ bearerToken, workspaceId, itemType }) => {
    const result = await executeApiCall(bearerToken, workspaceId, "list-items", (client) => client.listItems(itemType && itemType !== "All" ? itemType : undefined), { itemType });
    if (result.status === 'error') {
        return {
    ...
    const itemsList = items.map((item, index) => `${index + 1}. ${item.displayName} (${item.type})\n   ID: ${item.id}\n   Description: ${item.description || "No description"}\n   Modified: ${item.modifiedDate}`).join("\n\n");
    return {
        content: [{ type: "text", text: `Items in workspace:\n\n${itemsList}` }]
    };
});
server.tool("create-fabric-item", "Create a new item in Microsoft Fabric workspace", CreateItemSchema.shape, async ({ bearerToken, workspaceId, itemType, displayName, description }) => {
    const result = await executeApiCall(bearerToken, workspaceId, "create-item", (client) => client.createItem(itemType, displayName, description), { itemType, displayName, description });
    if (result.status === 'error') {
        return {
...
server.tool("update-fabric-item", "Update an existing item in Microsoft Fabric workspace", UpdateItemSchema.shape, async ({ bearerToken, workspaceId, itemId, displayName, description }) => {
    const updates = {};
    if (displayName)
    ...
    const result = await executeApiCall(bearerToken, workspaceId, "update-item", (client) => client.updateItem(itemId, updates), { itemId, updates });
    if (result.status === 'error') {
        return {
...
server.tool("delete-fabric-item", "Delete an item from Microsoft Fabric workspace", ItemOperationSchema.shape, async ({ bearerToken, workspaceId, itemId }) => {
    const result = await executeApiCall(bearerToken, workspaceId, "delete-item", (client) => client.deleteItem(itemId), { itemId });
    if (result.status === 'error') {
        return {
...
server.tool("get-fabric-item", "Get detailed information about a specific Microsoft Fabric item", ItemOperationSchema.shape, async ({ bearerToken, workspaceId, itemId }) => {
    const result = await executeApiCall(bearerToken, workspaceId, "get-item", (client) => client.getItem(itemId), { itemId });
    if (result.status === 'error') {
        return {
...
server.tool("execute-fabric-notebook", "Execute a notebook in Microsoft Fabric workspace", NotebookExecutionSchema.shape, async ({ bearerToken, workspaceId, notebookId, parameters, timeout }) => {
    const result = await executeApiCall(bearerToken, workspaceId, "execute-notebook", (client) => client.executeNotebook(notebookId, parameters), { notebookId, parameters, timeout });
    if (result.status === 'error') {
        return {
...
server.tool("submit-spark-job", "Submit a Spark job to run on a Lakehouse", SparkJobSchema.shape, async ({ bearerToken, workspaceId, lakehouseId, code, language, clusterConfig }) => {
    const result = await executeApiCall(bearerToken, workspaceId, "spark-job", (client) => client.submitSparkJob(lakehouseId, code, language, clusterConfig), { lakehouseId, code, language, config: clusterConfig });
    if (result.status === 'error') {
        return {
...
server.tool("get-job-status", "Get the status of a running job", JobStatusSchema.shape, async ({ bearerToken, workspaceId, jobId }) => {
    const result = await executeApiCall(bearerToken, workspaceId, "job-status", (client) => client.getJobStatus(jobId), { jobId });
    if (result.status === 'error') {
        return {
...
server.tool("create-fabric-notebook", "Create a new notebook in Microsoft Fabric workspace using the official API", {
    bearerToken: z.string().min(1).describe("Microsoft Fabric bearer token"),
    workspaceId: z.string().min(1).describe("Microsoft Fabric workspace ID"),
    displayName: z.string().min(1).max(256).describe("Display name for the notebook"),
    ...
    const result = await executeApiCall(bearerToken, workspaceId, "create-notebook", async (_client) => {
...
server.tool("get-workspace-spark-applications", "Get all Spark applications in a workspace for monitoring", SparkMonitoringBaseSchema.shape, async ({ bearerToken, workspaceId, continuationToken }) => {
    const result = await executeApiCall(bearerToken, workspaceId, "workspace-spark-applications", (client) => client.listItems("All"), // Use available method as fallback
    ...
    const items = result.data?.value || [];
    const sparkItems = items.filter((item) => item.type === "Notebook" || item.type === "Lakehouse" || item.displayName.toLowerCase().includes("spark"));
    ...
    const itemsList = sparkItems.slice(0, 10).map((item, index) => `${index + 1}. ${item.displayName} (${item.type})\n   ID: ${item.id}\n   Modified: ${item.modifiedDate}`).join("\n\n");
    return {
        content: [{
                type: "text",
...
server.tool("get-notebook-spark-applications", "Get all Spark applications/sessions for a specific notebook", SparkNotebookMonitoringSchema.shape, async ({ bearerToken, workspaceId, notebookId, continuationToken }) => {
    const result = await executeApiCall(bearerToken, workspaceId, "get-notebook-spark-applications", (client) => client.getNotebookSparkApplications?.(notebookId, continuationToken) ||
        client.simulateSparkApplications("notebook", notebookId), { notebookId, continuationToken });
    if (result.status === 'error') {
        return {
...
server.tool("get-spark-job-definition-applications", "Get all Spark applications/sessions for a specific Spark job definition", SparkJobDefinitionMonitoringSchema.shape, async ({ bearerToken, workspaceId, sparkJobDefinitionId, continuationToken }) => {
    const result = await executeApiCall(bearerToken, workspaceId, "get-spark-job-definition-applications", (client) => client.getSparkJobDefinitionApplications?.(sparkJobDefinitionId, continuationToken) ||
        client.simulateSparkApplications("job-definition", sparkJobDefinitionId), { sparkJobDefinitionId, continuationToken });
    if (result.status === 'error') {
        return {
...
server.tool("get-lakehouse-spark-applications", "Get all Spark applications/sessions for a specific lakehouse", SparkLakehouseMonitoringSchema.shape, async ({ bearerToken, workspaceId, lakehouseId, continuationToken }) => {
    const result = await executeApiCall(bearerToken, workspaceId, "get-lakehouse-spark-applications", (client) => client.getLakehouseSparkApplications?.(lakehouseId, continuationToken) ||
        client.simulateSparkApplications("lakehouse", lakehouseId), { lakehouseId, continuationToken });
    if (result.status === 'error') {
        return {
...
server.tool("get-spark-application-details", "Get detailed information about a specific Spark application", SparkApplicationOperationSchema.shape, async ({ bearerToken, workspaceId, livyId }) => {
    const result = await executeApiCall(bearerToken, workspaceId, "get-spark-application-details", (client) => client.getSparkApplicationDetails?.(livyId) ||
        client.simulateSparkApplicationDetails(livyId), { livyId });
    if (result.status === 'error') {
        return {
...
server.tool("cancel-spark-application", "Cancel a running Spark application", SparkApplicationOperationSchema.shape, async ({ bearerToken, workspaceId, livyId }) => {
    const result = await executeApiCall(bearerToken, workspaceId, "cancel-spark-application", (client) => client.cancelSparkApplication?.(livyId) ||
        client.simulateCancelSparkApplication(livyId), { livyId });
    if (result.status === 'error') {
        return {
...
server.tool("get-notebook-spark-application-details", "Get detailed information about a specific Spark application from a notebook session", NotebookSparkApplicationSchema.shape, async ({ bearerToken, workspaceId, notebookId, livyId, appId, attemptId }) => {
    const result = await executeApiCall(bearerToken, workspaceId, "get-notebook-spark-application-details", (client) => client.getNotebookSparkApplicationDetails?.(notebookId, livyId, appId, attemptId), { notebookId, livyId, appId, attemptId });
    if (result.status === 'error') {
        return {
...
server.tool("get-notebook-spark-application-jobs", "Get jobs for a specific Spark application from a notebook session", NotebookSparkApplicationSchema.shape, async ({ bearerToken, workspaceId, notebookId, livyId, appId, jobId, attemptId }) => {
    const result = await executeApiCall(bearerToken, workspaceId, "get-notebook-spark-application-jobs", (client) => client.getNotebookSparkApplicationJobs?.(notebookId, livyId, appId, jobId, attemptId), { notebookId, livyId, appId, jobId, attemptId });
    if (result.status === 'error') {
        return {
...
server.tool("get-spark-monitoring-dashboard", "Get comprehensive Spark monitoring dashboard for workspace", SparkDashboardSchema.shape, async ({ bearerToken, workspaceId, includeCompleted, maxResults }) => {
    const result = await executeApiCall(bearerToken, workspaceId, "get-spark-monitoring-dashboard", (client) => client.getSparkMonitoringDashboard?.(includeCompleted, maxResults) ||
        client.simulateSparkDashboard(includeCompleted, maxResults), { includeCompleted, maxResults });
    if (result.status === 'error') {
        return {
...
${dashboard.recentActivity.map((app, i) => `
 ...
 */
function analyzeSessionLogsWithLLM(sessionInfo, analysisType) {
    ...
    const performanceIndicators = logs.filter(log => log.includes('duration') || log.includes('time') || log.includes('memory') || log.includes('cpu'));
    const analysis = {
        summary: {
...
function extractPerformanceMetrics(statementData) {
    const metrics = {
        executionTime: null,
 ...
 */
function generateOptimizationRecommendations(statementData, performanceMetrics, analysisType) {
    const recommendations = {
        type: analysisType,
    ...
    if (statementData.output?.data?.['application/vnd.livy.statement-error+json']) {
        const errorData = statementData.output.data['application/vnd.livy.statement-error+json'];
 ...
 */
function filterSessionsByTimeRange(sessions, timeRange) {
    const now = new Date();
    let cutoffTime;
    switch (timeRange) {
        case '1h':
    ...
    return sessions.filter(session => {
        if (session.appInfo?.startTime) {
            return new Date(session.appInfo.startTime) >= cutoffTime;
        }
 ...
 */
function analyzeSessionHistory(session, analysisType) {
    const analysis = {
    ...
    if (session.appInfo?.startTime && session.appInfo?.endTime) {
        const start = new Date(session.appInfo.startTime).getTime();
        const end = new Date(session.appInfo.endTime).getTime();
 ...
 */
function analyzeMultipleSessionsHistory(sessions, analysisType, timeRange) {
    const analysis = {
        timeRange,
        analysisType,
 ...
 */
function analyzePerformanceFromLogs(logs) {
 ...
 */
function analyzeErrorsFromLogs(errorLogs) {
    const errorAnalysis = {
        errorTypes: {},
        criticalErrors: [],
        commonPatterns: []
 ...
 */
function generateSessionRecommendations(sessionInfo, logs) {
    const recommendations = [];
...
server.tool("get-livy-statement-enhanced", "Get Livy statement status with enhanced performance analysis and recommendations", LivyStatementOperationSchema.extend({
...
}).shape, async ({ bearerToken, workspaceId, lakehouseId, sessionId, statementId, includeAnalysis, analysisType }) => {
    try {
        const result = await executeApiCall(bearerToken, workspaceId, "get-livy-statement-enhanced", (client) => client.getLivyStatement(lakehouseId, sessionId, statementId), { lakehouseId, sessionId, statementId, includeAnalysis, analysisType });
        if (result.status === 'error') {
            return {
...
server.tool("analyze-livy-session-logs", "Analyze Livy session logs with LLM-powered performance insights and recommendations", LivySessionLogAnalysisSchema.shape, async ({ bearerToken, workspaceId, lakehouseId, sessionId, analysisType, useLLM }) => {
    try {
        const result = await executeApiCall(bearerToken, workspaceId, "analyze-livy-session-logs", (client) => client.getLivySession(lakehouseId, sessionId), { lakehouseId, sessionId, analysisType });
        if (result.status === 'error') {
            return {
...
server.tool("analyze-livy-statement-performance", "Analyze Livy statement execution with detailed performance metrics and LLM-powered optimization recommendations", LivyStatementLogAnalysisSchema.shape, async ({ bearerToken, workspaceId, lakehouseId, sessionId, statementId, analysisType, includeRecommendations }) => {
    try {
        const result = await executeApiCall(bearerToken, workspaceId, "analyze-livy-statement-performance", (client) => client.getLivyStatement(lakehouseId, sessionId, statementId), { lakehouseId, sessionId, statementId, analysisType });
        if (result.status === 'error') {
            return {
...
server.tool("analyze-livy-execution-history", "Analyze historical Livy execution patterns with trend analysis and performance insights", LivyExecutionHistorySchema.shape, async ({ bearerToken, workspaceId, lakehouseId, sessionId, timeRange, analysisType }) => {
    try {
        const result = await executeApiCall(bearerToken, workspaceId, "analyze-livy-execution-history", (client) => client.listLivySessions(lakehouseId), { lakehouseId, sessionId, timeRange, analysisType });
        if (result.status === 'error') {
            return {
...
server.tool("list-fabric-notebooks", "List all notebooks in a Microsoft Fabric workspace", BaseWorkspaceSchema.shape, async ({ bearerToken, workspaceId }) => {
    const result = await executeApiCall(bearerToken, workspaceId, "list-notebooks", (client) => client.listNotebooks(), {});
    if (result.status === 'error') {
        return {
    ...
    const notebooksList = notebooks.map((notebook, index) => `${index + 1}. ${notebook.displayName}\n   ID: ${notebook.id}\n   Description: ${notebook.description || "No description"}\n   Modified: ${notebook.modifiedDate || "Unknown"}`).join("\n\n");
    return {
        content: [{ type: "text", text: `Notebooks in workspace:\n\n${notebooksList}` }]
    };
});
server.tool("get-fabric-notebook", "Get details of a specific notebook in Microsoft Fabric workspace", NotebookOperationSchema.shape, async ({ bearerToken, workspaceId, notebookId }) => {
    const result = await executeApiCall(bearerToken, workspaceId, "get-notebook", (client) => client.getNotebook(notebookId), { notebookId });
    if (result.status === 'error') {
        return {
...
server.tool("update-fabric-notebook", "Update an existing notebook in Microsoft Fabric workspace", UpdateNotebookSchema.shape, async ({ bearerToken, workspaceId, notebookId, displayName, description }) => {
    const updates = {};
    if (displayName)
        updates.displayName = displayName;
    if (description)
    ...
    const result = await executeApiCall(bearerToken, workspaceId, "update-notebook", (client) => client.updateNotebook(notebookId, updates), { notebookId, updates });
    if (result.status === 'error') {
        return {
...
server.tool("delete-fabric-notebook", "Delete a notebook from Microsoft Fabric workspace", NotebookOperationSchema.shape, async ({ bearerToken, workspaceId, notebookId }) => {
    const result = await executeApiCall(bearerToken, workspaceId, "delete-notebook", (client) => client.deleteNotebook(notebookId), { notebookId });
    if (result.status === 'error') {
        return {
...
server.tool("get-fabric-notebook-definition", "Get the definition/content of a notebook in Microsoft Fabric workspace", GetNotebookDefinitionSchema.shape, async ({ bearerToken, workspaceId, notebookId, format }) => {
    const result = await executeApiCall(bearerToken, workspaceId, "get-notebook-definition", (client) => client.getNotebookDefinition(notebookId, format), { notebookId, format });
    if (result.status === 'error') {
        return {
    ...
    const parts = definition.parts.map((part, index) => `${index + 1}. ${part.path} (${part.payloadType})`).join("\n");
    return {
        content: [{
                type: "text",
...
server.tool("run-fabric-notebook", "Execute/run a notebook in Microsoft Fabric workspace", RunNotebookSchema.shape, async ({ bearerToken, workspaceId, notebookId, parameters, configuration }) => {
    const result = await executeApiCall(bearerToken, workspaceId, "run-notebook", (client) => client.runNotebook(notebookId, parameters, configuration), { notebookId, parameters, configuration });
    if (result.status === 'error') {
        return {
...
server.tool("create-spark-job-instance", "Create a Spark job instance from a Spark Job Definition", SparkJobInstanceSchema.shape, async ({ bearerToken, workspaceId, sparkJobDefinitionId, jobType }) => {
    const result = await executeApiCall(bearerToken, workspaceId, "create-spark-job-instance", (client) => client.createSparkJobInstance(sparkJobDefinitionId, jobType), { sparkJobDefinitionId, jobType });
    if (result.status === 'error') {
        return {
...
server.tool("execute-spark-job-definition", "Execute a Spark Job Definition with execution data", SparkJobDefinitionSchema.shape, async ({ bearerToken, workspaceId, sparkJobDefinitionId, jobType, executionData }) => {
    const result = await executeApiCall(bearerToken, workspaceId, "execute-spark-job-definition", (client) => client.executeSparkJobDefinition(sparkJobDefinitionId, jobType, executionData), { sparkJobDefinitionId, jobType, executionData });
    if (result.status === 'error') {
        return {
...
server.tool("get-spark-job-instance-status", "Get the status of a Spark job instance", SparkJobStatusSchema.shape, async ({ bearerToken, workspaceId, sparkJobDefinitionId, jobInstanceId }) => {
    const result = await executeApiCall(bearerToken, workspaceId, "get-spark-job-instance-status", (client) => client.getSparkJobInstanceStatus(sparkJobDefinitionId, jobInstanceId), { sparkJobDefinitionId, jobInstanceId });
    if (result.status === 'error') {
        return {
...
// ==================== LIVY SESSION TOOLS ====================
server.tool("create-livy-session", "Create a new Livy session for interactive Spark execution", LivySessionSchema.shape, async ({ bearerToken, workspaceId, lakehouseId, sessionConfig }) => {
    const result = await executeApiCall(bearerToken, workspaceId, "create-livy-session", (client) => client.createLivySession(lakehouseId, sessionConfig), { lakehouseId, sessionConfig });
    if (result.status === 'error') {
        return {
...
server.tool("get-livy-session", "Get the status of a Livy session", LivySessionOperationSchema.shape, async ({ bearerToken, workspaceId, lakehouseId, sessionId }) => {
    const result = await executeApiCall(bearerToken, workspaceId, "get-livy-session", (client) => client.getLivySession(lakehouseId, sessionId), { lakehouseId, sessionId });
    if (result.status === 'error') {
        return {
...
server.tool("list-livy-sessions", "List all Livy sessions for a lakehouse", BaseWorkspaceSchema.extend({
    lakehouseId: z.string().min(1).describe("Lakehouse ID")
}).shape, async ({ bearerToken, workspaceId, lakehouseId }) => {
    const result = await executeApiCall(bearerToken, workspaceId, "list-livy-sessions", (client) => client.listLivySessions(lakehouseId), { lakehouseId });
    if (result.status === 'error') {
        return {
...
server.tool("delete-livy-session", "Delete a Livy session", LivySessionOperationSchema.shape, async ({ bearerToken, workspaceId, lakehouseId, sessionId }) => {
    const result = await executeApiCall(bearerToken, workspaceId, "delete-livy-session", (client) => client.deleteLivySession(lakehouseId, sessionId), { lakehouseId, sessionId });
    if (result.status === 'error') {
        return {
...
server.tool("execute-livy-statement", "Execute a statement in a Livy session", LivyStatementSchema.shape, async ({ bearerToken, workspaceId, lakehouseId, sessionId, code, kind }) => {
    const result = await executeApiCall(bearerToken, workspaceId, "execute-livy-statement", (client) => client.executeLivyStatement(lakehouseId, sessionId, { code, kind }), { lakehouseId, sessionId, code, kind });
    if (result.status === 'error') {
        return {
...
server.tool("get-livy-statement", "Get the result of a statement execution", LivyStatementOperationSchema.shape, async ({ bearerToken, workspaceId, lakehouseId, sessionId, statementId }) => {
    const result = await executeApiCall(bearerToken, workspaceId, "get-livy-statement", (client) => client.getLivyStatement(lakehouseId, sessionId, statementId), { lakehouseId, sessionId, statementId });
    if (result.status === 'error') {
        return {
...
server.tool("create-livy-batch", "Create a Livy batch job", LivyBatchSchema.shape, async ({ bearerToken, workspaceId, lakehouseId, batchConfig }) => {
    const result = await executeApiCall(bearerToken, workspaceId, "create-livy-batch", (client) => client.createLivyBatch(lakehouseId, batchConfig), { lakehouseId, batchConfig });
    if (result.status === 'error') {
        return {
...
server.tool("get-livy-batch", "Get the status of a Livy batch job", LivyBatchOperationSchema.shape, async ({ bearerToken, workspaceId, lakehouseId, batchId }) => {
    const result = await executeApiCall(bearerToken, workspaceId, "get-livy-batch", (client) => client.getLivyBatch(lakehouseId, batchId), { lakehouseId, batchId });
    if (result.status === 'error') {
        return {
...
server.tool("check-fabric-auth-status", "Check current authentication status and configuration", z.object({}).shape, async () => {
...
server.tool("fabric_list_workspaces", "List all workspaces accessible to the user", ListWorkspacesSchema.shape, async ({ bearerToken, type, capacityId, name, state, continuationToken }) => {
    const result = await executeApiCall(bearerToken, authConfig.defaultWorkspaceId || "global", "list-workspaces", (client) => client.listWorkspaces(type, capacityId, name, state, continuationToken) ||
        client.simulateWorkspaces(type, capacityId, name, state), { type, capacityId, name, state, continuationToken });
    if (result.status === 'error') {
        return {
    ...
    const workspacesList = workspaces.map((workspace, index) => `${index + 1}. ${workspace.name} (${workspace.type})\n   ID: ${workspace.id}\n   State: ${workspace.state}\n   Capacity ID: ${workspace.capacityId || "No capacity"}`).join("\n\n");
    return {
        content: [{
                type: "text",
...
server.tool("fabric_find_workspace", "Find workspace by name and get its ID for use in other operations", FindWorkspaceSchema.shape, async ({ bearerToken, searchName }) => {
    const result = await executeApiCall(bearerToken, authConfig.defaultWorkspaceId || "global", "find-workspace", (client) => client.listWorkspaces(undefined, undefined, searchName) ||
        client.simulateWorkspaces(undefined, undefined, searchName), { searchName });
    if (result.status === 'error') {
        return {
            content: [{ type: "text", text: `Error: ${result.error}` }]
        };
    ...
    const workspaces = result.data?.workspaces || [];
    const matchingWorkspaces = workspaces.filter((workspace) => workspace.name.toLowerCase().includes(searchName.toLowerCase()));
    ...
    const workspacesList = matchingWorkspaces.map((workspace, index) => `${index + 1}. "${workspace.name}"\n   ID: ${workspace.id}\n   Type: ${workspace.type}\n   State: ${workspace.state}\n   Capacity ID: ${workspace.capacityId || "No capacity"}`).join("\n\n");
    return {
        content: [{
                type: "text",
...
server.tool("fabric_create_workspace", "Create a new workspace", {
    ...
    const result = await executeApiCall(bearerToken, authConfig.defaultWorkspaceId || "global", "create-workspace", (_client) => { throw new Error("Workspace management not yet implemented in fabric-client"); }, { name, description });
    if (result.status === 'error') {
        return {
            content: [{ type: "text", text: `Error: ${result.error}` }]
        };
 ...
 */
async function checkDataflowHealth(bearerToken, workspaceId, dataflowId) {
    try {
        const url = `https://api.fabric.microsoft.com/v1/workspaces/${workspaceId}/dataflows/${dataflowId}`;
        const response = await fetch(url, {
            headers: { 'Authorization': `Bearer ${bearerToken}` }
...
server.tool("create-fabric-dataflow", "Create a new Dataflow Gen2 in Microsoft Fabric workspace", CreateDataflowSchema.shape, async ({ bearerToken, workspaceId, displayName, description = "" }) => {
    try {
...
server.tool("list-fabric-dataflows", "List all Dataflow Gen2 in Microsoft Fabric workspace", ListDataflowsSchema.shape, async ({ bearerToken, workspaceId }) => {
    try {
        const url = `https://api.fabric.microsoft.com/v1/workspaces/${workspaceId}/dataflows`;
        const response = await fetch(url, {
            headers: {
...
server.tool("get-fabric-dataflow", "Get details of a specific Dataflow Gen2", GetDataflowSchema.shape, async ({ bearerToken, workspaceId, dataflowId }) => {
    try {
        const url = `https://api.fabric.microsoft.com/v1/workspaces/${workspaceId}/dataflows/${dataflowId}`;
        const response = await fetch(url, {
            headers: {
...
server.tool("update-fabric-dataflow", "Update Dataflow Gen2 name or description", UpdateDataflowSchema.shape, async ({ bearerToken, workspaceId, dataflowId, displayName, description }) => {
    try {
        const updateData = {};
        if (displayName)
...
server.tool("delete-fabric-dataflow", "Delete a Dataflow Gen2 from workspace", DeleteDataflowSchema.shape, async ({ bearerToken, workspaceId, dataflowId }) => {
    try {
        const url = `https://api.fabric.microsoft.com/v1/workspaces/${workspaceId}/dataflows/${dataflowId}`;
        const response = await fetch(url, {
            method: 'DELETE',
...
server.tool("monitor-dataflow-status", "Get comprehensive dataflow status with health monitoring and performance metrics", MonitorDataflowStatusSchema.shape, async ({ bearerToken, workspaceId, dataflowId, includeHealthMetrics }) => {
    try {
...
server.tool("perform-dataflow-health-check", "Perform comprehensive health check with scoring, analysis, and recommendations", PerformDataflowHealthCheckSchema.shape, async ({ bearerToken, workspaceId, dataflowId, checkDepth, includeRecommendations }) => {
    try {
...
server.tool("monitor-workspace-dataflows", "Get monitoring overview of all dataflows in workspace with health scoring", MonitorWorkspaceDataflowsSchema.shape, async ({ bearerToken, workspaceId, includeHealthChecks, sortBy }) => {
    try {
            ...
            if (includeHealthChecks) {
                try {
                    ...
                    if (healthMetrics.status === 'healthy')
                        overview.summary.healthyCount++;
                    else if (healthMetrics.status === 'warning')
                        overview.summary.warningCount++;
                    else if (healthMetrics.status === 'critical')
...
server.tool("generate-dataflow-monitoring-report", "Generate comprehensive monitoring report with insights and recommendations", GenerateDataflowMonitoringReportSchema.shape, async ({ bearerToken, workspaceId, dataflowId, reportType, outputFormat }) => {
    try {
        ...
        // Generate executive summary
        report.sections.executiveSummary = generateExecutiveSummary({ data: { basicInfo: dataflow } }, { healthCheck: report.sections.healthAssessment });
        // Generate recommendations
...
server.tool("start-continuous-dataflow-monitoring", "Start continuous monitoring session with real-time health checks and alerts", StartContinuousDataflowMonitoringSchema.shape, async ({ bearerToken, workspaceId, dataflowId, intervalMinutes, durationMinutes }) => {
...
// ====================================
server.tool("list-monitoring-sessions", "List all active monitoring sessions", z.object({}).shape, async () => {
...
server.tool("get-monitoring-session-status", "Get detailed status of a specific monitoring session", z.object({
    sessionId: z.string().describe("Monitoring session ID")
}).shape, async ({ sessionId }) => {
...
server.tool("stop-monitoring-session", "Stop a specific monitoring session", z.object({
...
server.tool("list-fabric-item-runs", "List all runs for a specific Fabric item with analysis", ListFabricItemRunsSchema.shape, async ({ bearerToken, workspaceId, itemId, continuationToken, top }) => {
    const result = await executeApiCall(bearerToken, workspaceId, "list-item-runs", async (client) => {
...
server.tool("get-fabric-item-run", "Get detailed information about a specific Fabric item run", GetFabricItemRunSchema.shape, async ({ bearerToken, workspaceId, itemId, jobInstanceId, includeSubactivities }) => {
    const result = await executeApiCall(bearerToken, workspaceId, "get-item-run", async (client) => {
        // Get run details
        ...
        if (process.env.DEBUG_MCP_RUN === '1') {
            const dbg = (...args) => console.error('[debug run-data]', ...args);
...
`;
            subactivityAnalysis.bottlenecks.forEach((bottleneck, index) => {
...
server.tool("list-fabric-run-subactivities", "List subactivities for a specific Fabric item run with analysis", ListFabricRunSubactivitiesSchema.shape, async ({ bearerToken, workspaceId, itemId, jobInstanceId, continuationToken, top }) => {
    const result = await executeApiCall(bearerToken, workspaceId, "list-run-subactivities", async (client) => {
    ...
    const data = result.data;
    const subactivities = data?.value || [];
...
server.tool("monitor-fabric-runs-dashboard", "Get comprehensive monitoring dashboard for recent runs across workspace", MonitorFabricRunsDashboardSchema.shape, async ({ bearerToken, workspaceId, hoursBack, includeSubactivities }) => {
    const result = await executeApiCall(bearerToken, workspaceId, "monitor-runs-dashboard", async (client) => {
        ...
        dashboard.recentFailures.sort((a, b) => new Date(b.startTime).getTime() - new Date(a.startTime).getTime());
...
server.tool("analyze-fabric-run-performance", "Perform detailed performance analysis on a specific run", AnalyzeFabricRunPerformanceSchema.shape, async ({ bearerToken, workspaceId, itemId, jobInstanceId, analysisType }) => {
    const result = await executeApiCall(bearerToken, workspaceId, "analyze-run-performance", async (client) => {
    ...
    const performanceMetrics = {
        ...
        longestActivity: subactivities.length > 0 ?
            subactivities
                .filter(a => a.endTime)
                .reduce((prev, curr) => calculateDuration(curr.startTime, curr.endTime) > calculateDuration(prev.startTime, prev.endTime) ? curr : prev) : null,
