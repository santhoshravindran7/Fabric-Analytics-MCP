...
export class CapacityManager {
    async getAccessToken() {
        const tokenOutput = execSync('az account get-access-token --resource https://api.fabric.microsoft.com --query accessToken -o tsv', { encoding: 'utf-8' });
        return tokenOutput.trim();
    }
    /**
     * Get Spark VCore specifications for a capacity SKU
     */
    getCapacitySpecs(sku) {
    ...
    async assignCapacity(workspaceId, capacityId) {
        const token = await this.getAccessToken();
        const response = await fetch(`https://api.fabric.microsoft.com/v1/workspaces/${workspaceId}/assignToCapacity`, {
            method: 'POST',
            headers: {
     ...
     */
    async getWorkspaceCapacity(workspaceId) {
        const token = await this.getAccessToken();
